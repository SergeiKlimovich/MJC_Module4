...
public final class UserValidator {
    ...
    private static void validateName(String name) {
        if (Objects.isNull(name)) {
            throw new InvalidDataException("To register, enter a name");
        }
        if (name.length() < MIN_LENGTH_NAME || name.length() > MAX_LENGTH_NAME) {
            throw new InvalidDataException("The length of the name can not be less than 2 characters and more than 45 characters");
        }
    }

    private static void validateEmail(String email) {
        if (Objects.isNull(email)) {
            throw new InvalidDataException("To register, enter a email");
        }
        if (!email.matches(EMAIL_REGEXP)) {
            throw new InvalidDataException("Pattern of email : xx@xx.xx");
        }
    }

    private static void validatePassword(String password) {
        if (Objects.isNull(password)) {
            throw new InvalidDataException("To register, enter a password");
        }
        if (password.length() < MIN_LENGTH_PASSWORD || password.length() > MAX_LENGTH_PASSWORD) {
            throw new InvalidDataException("The length of the password can not be less than 5 characters and more than 20 characters");
        }
    }
}
